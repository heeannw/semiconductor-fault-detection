...
#### Gemini-based natural-language explanation layer (optional, `backend/app/llm_explain.py`)

Everything above (rule-based diagnosis, the trained classifier) produces **deterministic** results. This adds a free-tier Gemini API layer that lets a user ask free-form questions about a reading — but deliberately **never lets Gemini make a new judgment**. The rule-based diagnosis and classifier output are baked into the prompt as established facts, with an explicit instruction to answer only from those facts (RAG-style) — a design choice made specifically because a semiconductor root-cause diagnosis is the wrong place to trust an LLM's own guesses.

- If `GEMINI_API_KEY` is missing from `.env` (not committed — see `.env.example`), only this feature degrades gracefully to a 503, same pattern as every other ML-dependent feature in this project.
- Only called once per explicit user question — no automatic polling, no retry loops, to avoid burning free-tier quota unnecessarily.
- The test suite (`backend/tests/test_process_explain.py`) never calls the real Gemini API — `gemini_available`/`explain` are monkeypatched, and the tests verify the server recomputes the diagnosis itself server-side (rather than trusting whatever the client sent) before grounding the prompt in it. No key is configured in CI, so this feature is always exercised in its disabled state there.
- Wired into the dashboard via an "Ask AI" input on the `ProcessCard` component.
...
- Run locally: `.venv/Scripts/python -m uvicorn backend.app.main:app --port 8000` (from the project root); Swagger UI at `/docs`. To use the Gemini explanation feature, copy `.env.example` to `.env` and fill in `GEMINI_API_KEY` (optional — everything else works fine without it).
...
- [x] Gemini-based natural-language explanation layer (`backend/app/llm_explain.py`, `POST /api/process/explain`, optional) — free-tier API, grounds every answer in the rule-based/AI diagnosis output (RAG-style, no new judgments generated). Degrades gracefully to a 503 without a key; tests never call the real API, verified via monkeypatch
