"""공정 파라미터가 규격을 벗어났을 때 원인 후보/조치를 제안하는 규칙 기반 진단.
...
반도체 공정 도메인 지식에 기반한 원인 후보/조치를 정적 규칙으로 제안할 수 있다.
...
DIAGNOSIS_TEMPLATES: dict[tuple[str, str, Direction], DiagnosisTemplate] = {
    ("wafer_fabrication", "temperature", "high"): DiagnosisTemplate(
        "웨이퍼 제조 온도 상한 초과", "히터 과열 또는 온도 컨트롤러 드리프트 — 결정 결함 위험",
